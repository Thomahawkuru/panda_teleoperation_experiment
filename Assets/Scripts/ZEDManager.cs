...
public class ZEDManager : MonoBehaviour
{
    ...
    private string svoOutputFilePath;
    ...
    private void UpdateRecording()
    {
        if ((Input.GetKeyDown(toggleLoggingKey)) || (Input.GetKeyDown(toggleCountdownKey)))
        {
            if (!recording)
            {
                ...
                svoOutputFilePath = subjectDataFolder + "/" + svoOutputFileName;
                zedCamera.EnableRecording(svoOutputFilePath, svoOutputCompressionMode, svoOutputBitrate, svoOutputTargetFPS, svoOutputTranscodeStreaming);
                recording = true;
                Debug.Log("Started SVO Recording at: " + svoOutputFilePath);
            }
            else
            {
                zedCamera.DisableRecording();
                recording = false;
                Debug.Log("Stopped SVO Recording");
            }
            return;
        }
    }
